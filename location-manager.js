...
export class LocationManager {
    ...
    _sanitizePost(post = {}) {
        return {
            ...
            source: ['place', 'chat', 'local', 'animal'].includes(post.source) ? post.source : '',
            voice: ['dry', 'rant', 'burst', 'chatty', 'soft', 'info'].includes(post.voice) ? post.voice : '',
            imagePrompt: safeText(post.imagePrompt, 220),
            photoSource: post.photoSource === 'grounded_public_v1' ? 'grounded_public_v1' : '',
            factIds: Array.isArray(post.factIds) ? post.factIds.slice(0, 12).map(v => safeText(v, 20)).filter(Boolean) : [],
