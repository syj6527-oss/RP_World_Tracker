...
export class MapRenderer {
    ...
    _init() {
        ...
        this.svg.addEventListener('pointerdown', e => this._fantasyPointerDown(e));
        this.svg.addEventListener('pointermove', e => this._fantasyPointerMove(e));
        this.svg.addEventListener('pointerup', e => this._fantasyPointerUp(e));
        this.svg.addEventListener('pointercancel', () => this.cancelFantasyGesture());
        this.svg.addEventListener('lostpointercapture', () => this.cancelFantasyGesture());
        this.svg.addEventListener('contextmenu', e => { if (this.fantasyMode) e.preventDefault(); });
        this.svg.addEventListener('mousedown', e => { if (!this.fantasyMode) this._onDown(e); });
        this.svg.addEventListener('mousemove', e => { if (!this.fantasyMode) this._onMove(e); });
        this.svg.addEventListener('mouseup', () => { if (!this.fantasyMode) this._onUp(); });
        this.svg.addEventListener('mouseleave', () => { if (!this.fantasyMode) this._onUp(); });
        this.svg.addEventListener('wheel', e => { e.preventDefault(); this._zoom(e.deltaY > 0 ? 1.1 : 0.9, e); }, { passive: false });
        this.svg.addEventListener('touchstart', e => {
            if (!this.fantasyMode) this._touchStart(e);
            else if (e.touches.length === 2) {
                e.preventDefault(); this.cancelFantasyGesture(); this._pinch = this._pinchDist(e);
            }
        }, { passive: false });
        this.svg.addEventListener('touchmove', e => {
            if (!this.fantasyMode) this._touchMove(e);
            else if (e.touches.length === 2 && this._pinch) {
                this._vbManual = true; this._touchMove(e);
            }
        }, { passive: false });
        this.svg.addEventListener('touchend', e => {
            if (!this.fantasyMode) this._touchEnd(e);
            else this._pinch = null;
        });
        this.svg.addEventListener('touchcancel', () => { this._pinch = null; this.cancelFantasyGesture(); });
    }
    // Pointer events isolate fantasy gestures from synthetic touch/mouse clicks.
    _fantasyPointerDown(e) {
        if (!this.fantasyMode || (e.button != null && e.button !== 0)) return;
        if (e.isPrimary === false) { this.cancelFantasyGesture(); return; }
        if (this._fantasyGesture) { this.cancelFantasyGesture(); return; }
        const pt = this._svgPt(e);
        this._fantasyGesture = { id: e.pointerId, x: e.clientX, y: e.clientY,
            pt, nodeId: this._hitTest(pt), held: false, moved: false,
            vx: this.vb.x, vy: this.vb.y, chatId: this.lm.currentChatId };
        try { this.svg.setPointerCapture(e.pointerId); } catch (_) {}
        this._fantasyHoldTimer = setTimeout(() => {
            if (this._fantasyGesture) this._fantasyGesture.held = true;
        }, 550);
    }
    _fantasyPointerMove(e) {
        const g = this._fantasyGesture;
        if (!g || e.pointerId !== g.id) return;
        const dx = e.clientX - g.x, dy = e.clientY - g.y;
        if (Math.hypot(dx, dy) > 10) {
            g.moved = true; g.held = false;
            clearTimeout(this._fantasyHoldTimer);
        }
        if (g.moved && !g.nodeId && !this._movingNodeId) {
            const rect = this.svg.getBoundingClientRect();
            this.vb.x = g.vx - dx * this.vb.w / rect.width;
            this.vb.y = g.vy - dy * this.vb.h / rect.height;
            this._vbManual = true; this._applyVB();
        }
    }
    _fantasyPointerUp(e) {
        const g = this._fantasyGesture;
        if (!g || e.pointerId !== g.id) return;
        this.cancelFantasyGesture();
        if (!this.fantasyMode || g.chatId !== this.lm.currentChatId || g.moved) return;
        if (this._movingNodeId) {
            const loc = this.lm.locations.find(l => l.id === this._movingNodeId);
            this._movingNodeId = null;
            if (loc) {
                const pos = { x: Math.round(g.pt.x), y: Math.round(g.pt.y), _manualXY: true };
                Object.assign(loc, pos);
                this.lm.updateLocation(loc.id, pos);
                this._savePinPos(loc.id, pos.x, pos.y);
                this._vbManual = true; this.render();
            }
        } else if (g.held && g.nodeId) {
            this._movingNodeId = g.nodeId;
            this.onMoveRequest?.(g.nodeId, this.lm.locations.find(l => l.id === g.nodeId)?.name || '');
        } else if (g.held) {
            // Prompt after release; never open it under an active finger.
            this.onLongPress?.(g.pt.x, g.pt.y);
        } else if (g.nodeId) {
            this.onLocationClick?.(g.nodeId);
        }
    }
    cancelFantasyGesture() {
        clearTimeout(this._fantasyHoldTimer);
        const id = this._fantasyGesture?.id;
        this._fantasyGesture = null;
        if (id != null) { try { this.svg.releasePointerCapture(id); } catch (_) {} }
    }
    ...
    _renderFantasyWorld() {
        ...
        if (curLoc && !curLoc._manualXY && curLoc.x === 0 && curLoc.y === 0) {
            curLoc.x = 300; curLoc.y = 280;
            this.lm.updateLocation(curLoc.id, { x: curLoc.x, y: curLoc.y });
        }
        ...
        svg += this._parchmentBackgroundSvg(this.vb);
        ...
        for (const loc of locations) {
            ...
            svg += this._parchmentPin(loc, current);
        ...
        if (!locations.length) {
            svg += `<text x="${this.vb.x + this.vb.w / 2}" y="${this.vb.y + this.vb.h / 2}" text-anchor="middle" fill="#5D4037" font-size="14" font-family="-apple-system,'Noto Sans KR',sans-serif">빈 곳을 꾹 누르고 놓으면 장소를 만들 수 있어요</text>`;
        }
        svg += this._compassRose(this.vb.x + 34, this.vb.y + this.vb.h - 34);
        this.svg.innerHTML = svg;
    }

    _parchmentBackgroundSvg(vb) {
        // Only paper: no decorative terrain, invented roads, or imaginary settlements.
        return `<g pointer-events="none"><rect x="${vb.x - 10000}" y="${vb.y - 10000}" width="20000" height="20000" fill="url(#wt-fmap-paper)"/><rect x="${vb.x - 10000}" y="${vb.y - 10000}" width="20000" height="20000" fill="url(#wt-fmap-grain)"/></g>`;
    }

    _parchmentPin(loc, current) {
        const color = current ? '#A34232' : '#846642';
        return `<g class="wt-location-node" data-id="${this._escapeSvgText(loc.id)}" transform="translate(${loc.x},${loc.y})">
            <circle r="24" fill="transparent" pointer-events="all"/>
            <path d="M0,8 C-3,3 -11,-6 -11,-13 A11,11 0 1,1 11,-13 C11,-6 3,3 0,8Z" fill="${color}" stroke="#FFF1D5" stroke-width="2"/>
            <circle cy="-13" r="3.5" fill="#FFF1D5"/>
        </g>`;
    }
